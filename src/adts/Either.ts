...
export class Either<L, R>
  ...
  reduce<T>(reducer: (accumulator: T, value: R) => T, initialValue: T): T {
    return this.isLeft_()
      ? initialValue
      : reducer(initialValue, this.asRight().value)
  }
  ...
  caseOf<T>(patterns: EitherPatterns<L, R, T>): T {
    if ('_' in patterns) {
      ...
      return this.isLeft_()
        ? patterns.Left(this.asLeft().value)
        : patterns.Right(this.asRight().value)
    }
  }
  ...
  either<T>(ifLeft: (value: L) => T, ifRight: (value: R) => T): T {
    return this.isLeft_()
      ? ifLeft(this.asLeft().value)
      : ifRight(this.asRight().value)
  }
