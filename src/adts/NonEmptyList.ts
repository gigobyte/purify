...
const isNonEmpty = <T>(list: T[]): list is NonEmptyList<T> => list.length > 0
...
const head = <T>(list: NonEmptyList<T>): T => list[0]

/** The same function as \`List#last\`, but it doesn't return a Maybe as a NonEmptyList will always have a last element */
const last = <T>(list: NonEmptyList<T>): T => list[list.length - 1]

const NonEmptyListConstructor = <T extends NonEmptyListCompliant>(
  list: T
): NonEmptyList<T[number]> => (list as any) as NonEmptyList<T[number]>

export const NonEmptyList: INonEmptyList = Object.assign(
  NonEmptyListConstructor,
  { fromArray, fromTuple, unsafeCoerce }
)
